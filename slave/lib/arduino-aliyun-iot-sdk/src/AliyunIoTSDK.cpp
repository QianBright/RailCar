    ...
    for (int i = 0; i < DATA_CALLBACK_SIZE; i++)
    {
        if (poniter_array[i].key)
        {
            ...
            if (hasKey)
            {
                ...
                serializeJsonPretty(parm["params"], Serial);
                Serial.println();
                // int32_t params = parm["params"].as<int>();
                // // const char *params = parm["params"].as<char *>();
...
{
    Serial.println();
    Serial.print("ALINK_TOPIC_PROP_POST: ");
    Serial.println(ALINK_TOPIC_PROP_POST);
    char jsonBuf[1024];
